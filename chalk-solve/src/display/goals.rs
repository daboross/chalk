...
impl<I: Interner> RenderAsRust<I> for ProgramClause<I> {
    fn fmt(&self, s: &InternalWriterState<'_, I>, f: &mut Formatter<'_>) -> Result {
        ...
        // TODO
        // are bindings created within the if block accessible outside of the if block, i.e.
        // if (forall <'a> { WellFormed(foo(&'a u32)) }) { WellFormed(bar(&'a u32)) }
        // If so, we will need to add the debrujin index to `s` in the caller
        let s = &s.add_debrujin_index(None);
